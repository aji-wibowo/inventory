  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      $('#bTambah').click(function(e){
        ...
        form.attr('action', '<?= base_url() ?>admin/supplier/ajax/save');
        form[0].reset();
      ...
      $(document).on('click', '.bEdit', function(e){
        ...
        var form = $('#formTambahSupplier');
        form.attr('action', '<?= base_url() ?>admin/supplier/ajax/update/' + dataId);
        form[0].reset();
        $('#ourModal .modal-title').html('Edit Supplier');

        $.ajax({
          url : '<?= base_url('admin/supplier/ajax/get/id') ?>',
          type : 'POST',
          data : {id : dataId},
          success : function(r){
            obj = $.trim(r);
      ...
      $(document).on('click', '.bDelete', function(e){
        ...
        .then((willDelete) => {
          if (willDelete) {
            $.ajax({
              url : '<?= base_url('admin/supplier/ajax/delete/') ?>' + id,
              type : 'post',
      ...
      function load(){
        $.ajax({
          url : '<?= base_url('admin/supplier/ajax/get/all') ?>',
          type : 'post',
          success : function(r){
            r = $.trim(r);
            r = $.parseJSON(r);
