...
<section class="content">
  ...
  <script type="text/javascript">
    ...
    $('#bTambah').click(function(e){
      ...
      form.attr('action', '<?= base_url() ?>admin/satuan/ajax/save');
      form[0].reset();
    ...
    $(document).on('click', '.bEdit', function(e){
      ...
      var form = $('#formTambahSatuanBarang');
      form.attr('action', '<?= base_url() ?>admin/satuan/ajax/update/' + dataId);
      form[0].reset();
      $('#ourModal .modal-title').html('Edit Satuan Barang');

      $.ajax({
        url : '<?= base_url('admin/satuan/ajax/get/id') ?>',
        type : 'POST',
        data : {id : dataId},
        success : function(r){
          obj = $.trim(r);
    ...
    $(document).on('click', '.bDelete', function(e){
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            url : '<?= base_url('admin/satuan/ajax/delete/') ?>' + id,
            type : 'post',
    ...
    function load(){
      $.ajax({
        url : '<?= base_url('admin/satuan/ajax/get/all') ?>',
        type : 'post',
        success : function(r){
          r = $.trim(r);
          r = $.parseJSON(r);
